...
4. 单位说明（务必严格遵守，避免混淆）：
   - 总股本、流通股本：akshare 常以【万股】为单位返回，换算时须 ÷10000 得到亿股；
     存储时 unit 填写"亿股"，value 为换算后的数值
   - 每股指标（EPS、每股净资产、每股经营现金流等）：单位为"元/股"，
     不得将其当作"亿元"使用；notes 中注明"每股指标，不可作为总量"
   - 比率/率类指标（毛利率、净利率、ROE 等）：unit 填"%"，value 为百分数形式（如 91.2）
5. 仅输出 JSON，不加任何解释
